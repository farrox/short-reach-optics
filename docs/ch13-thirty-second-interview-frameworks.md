...
**Deep dive.** Appendix A.10.2, Table 9.2, Appendix A.8.7.
...
**Key concepts.** Fleet economics and scope pick the owner before FA. Pulling units without a bucket wastes the only failing state you had (Appendix A.6, §10.10).
...
**Key concepts.** Intermittents die when the evidence is destroyed. NFF is often a triage failure, not a healthy part (§10.10).
...
Add a register only if it changes contain, derate, RMA, or FA ownership. Full tradeoff: §10.10.
...
*Criteria:* allocate margin where uncertainty is highest; do not maximize every ledger (§7.19, Chapter 9).
...
*Criteria:* each field needs a decision owner and a reaction plan (Appendix C.14, §10.10).

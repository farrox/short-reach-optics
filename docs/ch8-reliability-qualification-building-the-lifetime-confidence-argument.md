...
Qualification bounds life risk for representative hardware. Whether production can reproduce that result, screen escapes, and hold SPC is developed in Chapter 9. Failures that pass qual but fail field usually sit in derating policy, connector contamination, or a manufacturing coverage gap (§5.13, §11.16, §9.6).
...
Arrhenius math, derating, and the worked FIT example live in §5.13. This section is the mechanism catalog: how each failure shows up in ATP and telemetry, and which triage bucket owns it (§11.16). Do not run process CAPA on a wear-out part, and do not burn FIT math on a dirty connector.
...
**Table 8.2.** Wear-out and packaging mechanisms versus observables. Arrhenius projection and derating for the laser rows: §5.13. Electronics stress qualification: §8.4. Connector reliability: §8.7. Field classification workflow: §11.16.

### Reading the wear-out map

Table 8.2 is a triage map, not a life calculator. Classify before corrective action. Reliability rows need life models and derate. Performance rows need plant and control fixes. Manufacturing and packaging rows need process and ATP screens. Mixing buckets wastes CAPA (§11.16). Arrhenius projection lives in §5.13; this section teaches how to read the map in three families.

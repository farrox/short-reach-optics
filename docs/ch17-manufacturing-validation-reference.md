...
Program-phase labels (EVT, DVT, PVT, MP) are owned in §9.4, Table 9.3. They are not a second lifecycle. The table below is a manufacturing-evidence lookup: which release call the gate evidence supports when the program uses those names.
...
**Table G.1.** NPI gates as a manufacturing evidence lookup. Pilot sits between PVT and MP. MP is sustained control, not fleet monitoring alone. Phase definitions: §9.4. Argument structure: Chapter 9.

## Laser-bearing production-control checklist

Every second of ATP costs line capacity. Every omitted screen leaves risk. Buy evidence in categories that cannot substitute for each other. Table G.2 is a working checklist for an EML pluggable or an ELSFP CW module. Customize limits from the datasheet and the link budget; do not invent numbers in the ATP itself. Control-selection logic lives in Appendix G.14, Table G.6.
...
SPC on LIV, SMSR, RIN, TDECQ, and mate-cycle yield; golden-unit tracking; gauge R&R; incoming quality; first-article / FAIR after tooling or site change (Appendix G.11).

##### Fleet controls.

Telemetry, lot traceability, RMA codes split by mechanism, and recurrence monitoring (§10.10). These catch what ATP never saw.
...
This is a procedure-level sketch, not a statistics course. Worksheets, acceptance tables, and attribute studies live in the AIAG measurement-systems analysis reference . Why measurement-system analysis precedes yield interpretation: Appendix G.11.
...
Use this checklist when teaching or auditing a production station. Background and ANOVA sketch: Appendix G.11, Appendix G.3.
...
- Qualification re-entry depth when life or environmental mechanisms are affected (Chapter 9, Appendix F).
...
1.  **Requirements / PRD slice for the laser path:** fill Table 7.4, §7.6 (power class, grid, RIN@ORL, SMSR, derating, CMIS, FIT). Version it with the production control plan.
...
The following sections are the former manufacturing-validation chapter body, kept as reference depth. Start from Chapter 9 for the interview spine. Extended drills remain in Appendix G.20.1, Appendix M.10.

## Rapid Interview Checks

These are short prompts for self-test. Worked spoken answers live in Appendix G.20.1.
...
*Check.* Otherwise station drift, golden aging, or bias masquerades as product or supplier failure (Appendix G.11).
...
*Check.* Only when evidence ties the mechanism to a preventable production or test-control gap. Otherwise wear-out, interop, install, service, software, or residual latent risk (Appendix G.18, Chapter 10).


<div class="nav-links">
  <a href="ch16-reliability-qualification-reference">&larr; Previous</a>
  <a href="./">Table of Contents</a>
  <a href="ch18-ai-fabric-context">Next &rarr;</a>
</div>

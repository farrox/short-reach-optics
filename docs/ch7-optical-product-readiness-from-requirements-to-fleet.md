...
*Read first:* product-readiness lifecycle prose; evidence disciplines; bring-up; core measurements; CMIS sequence; production-like corners; fleet triage.

*Deep dive:* instrument theory of operation; Method A/B link-budget accounting; CMIS register map detail.

*Reference:* evidence-discipline table, readiness Stage/Question/Evidence/Decision table, bring-up checklist, corner checklist.
...
Companies may organize these activities under phase names such as EVT, DVT, or PVT, but those labels do not replace the technical definitions. Organizational team names are not used as engineering definitions in this book. Abbreviations are collected in Appendix G.
...
**Table 7.2.** Evidence disciplines and the decisions they can honestly support. Abbreviations: Appendix G.
...
The steps are ordered by the questions they answer, not by a rule that one department must finish before another begins. Architecture review, test development, supplier qualification, reliability planning, factory preparation, and telemetry design often overlap in calendar time. The important requirement is that later decisions do not claim evidence that earlier work has not established.

Examples of healthy overlap: qualification planning can begin during architecture review; chamber stress can run while interoperability testing continues; ATP development can begin during characterization; factory measurement-system studies can begin before final production freeze; pilot telemetry must be designed before pilot deployment; fleet controls should be defined before unrestricted ramp. Parallel execution is healthy. Ambiguous evidence ownership is not.

### One lifecycle, beginning before hardware

Product readiness begins before hardware and continues through fleet learning. It begins when the team decides what the product must accomplish and whether the proposed architecture can plausibly accomplish it. Requirements define success. Architecture work determines whether optical power, noise, thermal, electrical, reliability, manufacturing, and service assumptions can close together before tooling and hardware make changes expensive.
...
Requirement verification and system validation place that behavioral model inside the intended system. The question is no longer merely how the module behaves in isolation, but whether frozen requirements hold and enough headroom remains across supported hosts, peers, firmware, cable plants, thermal loading, and operating corners. This is where the team defines the real shipping envelope.

Reliability qualification is the next readiness evidence stream: whether time, stress, and environmental exposure cause permanent degradation. Manufacturing validation then asks whether the qualified result can be reproduced across lots, tools, suppliers, and production measurements. Qualification supports confidence in the design; manufacturing validation supports confidence in the process.
...
This is one product-readiness lifecycle. Simulation, bench measurement, qualification stress, and field evidence are not competing paths. They are different evidence sources selected within the appropriate step (Appendix D.16, Appendix D.2).
...
The first readiness step is not a measurement. It is defining what success means so you do not qualify or validate the wrong product. Without a requirements slice you do not know which BER, envelope, lifetime, volume, or deployment environment later measurements must close.

Write performance, environment, reliability, manufacturing, and operational requirement classes with owners and, where applicable, named planes (§1.1, Table 5.4). Weak answer: "We test BER." Strong answer: first define target BER, operating envelope, lifetime, manufacturing volume, and deployment environment; then design the readiness program around the risks. The evidence here is a signed contract, not a bench plot.
...
**Decision:** Architecture target and readiness scope, or hold until success is defined.\
...
Determine whether the architecture can meet the requirements before hardware exists. Product readiness starts on paper and in models (§1.1, Table 5.10). Even with clear requirements, optical power, thermal budget, electrical margin, reliability target, and manufacturing cost may not close together.
...
**Interview trap:** Skipping reliability and manufacturing cost until the program's DVT phase.
...
Confirm emit, receive, CDR lock, and usable pre-FEC BER at named planes before arguing about margin or life (§7.11, Table 7.5). The output is a known configuration that becomes the baseline for every later comparison, not a claim that the product is ready.
...
Map temperature, voltage, optical stress, and unit/lot distributions. On the transmitter path, measure TDECQ, OMA, ER, and level linearity; with component access, add LIV, RIN, and SMSR (§7.6, §7.9, §5.7). Characterization still studies the product largely as an isolated object.
...
Characterization tells how the product moves. Step 5 closes two related but different claims. Verification asks whether measured evidence meets frozen requirements. System validation asks whether the complete product is suitable for its intended use across the supported ecosystem. Verification and validation often use related evidence, but they support different claims. Verification closes a requirement. System validation closes intended use.

##### Verify requirements and margin.

Work from frozen requirements, named reference planes, BER and FEC conditions, transmitter and receiver limits, and stated optical, electrical, thermal, and control margins. Include measurement uncertainty. Record pass, fail, or conditional pass with traceability from evidence to requirement. Margin testing is verification when it checks a defined margin requirement under specified conditions.

A product does not fail because it reaches a nominal limit. It fails because all margins are consumed. Ask how much capability survives when temperature, voltage, loss, ORL, aging intent, and manufacturing variation stack under production-like stress (§7.11, §5.19). Compare pre-FEC BER, telemetry, retrain count, and control headroom against the characterization baseline. Track optical, electrical, thermal, reliability, and manufacturing margin categories separately, then stack once. Do not subtract the same physical effect twice under two names. Quiet-bench margin is not rack margin.
...
The same margin experiment can support system validation when it demonstrates that production-representative hosts, peers, firmware, and fiber plants work together across the supported envelope. Intended hosts, peer modules, firmware, fiber plants, chassis loading, management states, startup and recovery, supported environmental corners, and operational traffic belong here.

A component can meet isolated requirements and still fail as a system. Prove supported host, peer, firmware, and channel combinations retain required headroom. Margin on a golden host does not prove the supported ecosystem. Exercise host ASIC / SerDes, peer module or second source, firmware and CMIS revision, faceplate temperature and airflow, and fiber/cable plant. Assign failures to host, peer, software, environment, or channel before changing laser bias tables.
...
**Representative evidence:** Traceable requirement results with conditions and uncertainty; cliff locations and remaining headroom by ledger; margin waterfall without double-counted penalties; loaded-corner results (Table 7.6); supported combination list and documented restrictions.\
**Exit:** Frozen requirements are verified at the named plane, failure cliffs and remaining headroom are known at loaded corners, and supported host/peer/firmware/channel combinations retain required headroom or a documented restriction defines where the product may ship.\
**Decision:** Approve the shipping envelope and supported ecosystem, restrict deployment or SKUs, or send the design back.\
**Interview trap:** Calling golden-host margin the ecosystem exit, conflating requirement verification with intended-use validation, or subtracting the same impairment twice under two names.

Once the present-day shipping envelope is understood, reliability qualification can ask whether time and exposure change it.

### Step 6: Qualify reliability

Reliability qualification is a separate evidence discipline within the product-readiness lifecycle. It is not simply a more severe version of system validation. System validation demonstrates suitability across the intended operating envelope. Reliability qualification asks whether time, cycling, handling, moisture, mechanical exposure, electrical stress, or other credible mechanisms cause permanent degradation that threatens that suitability. Qualification evidence may begin before system validation is complete and may overlap program phases. Its placement as Step 6 describes the logical evidence sequence, not a prohibition against schedule overlap.
...
Manufacturing validation does not re-prove system suitability or product lifetime. It demonstrates that production-intent materials, processes, tools, software, fixtures, measurements, suppliers, and controls can reproduce the design for which those earlier claims were made. A hand-built design may pass system validation and reliability qualification while failing manufacturing validation because the production line cannot achieve the required distribution, measurement correlation, yield, traceability, or defect coverage.

The factory is part of the design. Reliability qualification supports the design claim. Manufacturing validation supports the process claim: build it repeatedly, measure it repeatedly, and detect bad units. Engineering samples often receive unusual attention; volume readiness requires manufacturing distributions, not only the best units.

This is typically the program's PVT question: whether yield, process control, and ATP coverage survive lot-to-lot variation (§9.5, §9.2, Table 9.2). DVT-phase system evidence belongs earlier; do not park it inside this step. Prove ATP/sample/SPC[^22] coverage against known escapes with replay, separation, and production repeatability. Measurement-system studies use gauge R&R[^23] when station confidence is in doubt. Burn-in[^24] is optional and mechanism-specific; it is not a substitute for Step 6.
...
A pilot is not merely a small production shipment. It is a controlled experiment designed to determine whether readiness assumptions from characterization, system validation, reliability qualification, and manufacturing validation remain true in deployment. Lab and factory evidence may still miss install practice, traffic mix, or environment.

Use known serial numbers and lots, representative hosts, enhanced telemetry, and explicit success and rollback criteria (§7.14). Telemetry should reveal whether BER, FEC behavior, retrains, temperature, optical power, lane behavior, and cohort rates match expected distributions.
...
Product readiness does not end at shipment. Keep escapes and drift detectable after release (§7.14, Table 7.7). Unknown failure modes, aging, supplier drift, and environmental effects appear only at scale. Every telemetry field must answer what decision it enables. Trend and disagreement alarms catch dying units; hard thresholds catch dead ones.
...
At any step, use the least expensive evidence that can responsibly answer the current question. An experiment is not inherently characterization, verification, validation, or qualification. Its category is determined by the claim being made.

- A sweep that maps behavior is characterization.

- A measured point compared with a requirement is verification.

- A production-representative system combination used to demonstrate intended use is system validation.

- A stress exposure used to evaluate permanent mechanism-driven degradation is reliability qualification.

- A station study used to demonstrate factory measurement capability is manufacturing validation.
...
The evidence source does not determine the lifecycle step. The engineering question does. Simulation may support Step 2 architecture review, while a correlated model may also support a Step 5 margin decision. A chamber experiment may support characterization or qualification depending on whether it measures reversible operation or permanent degradation. Reusing data across claims is acceptable only when the population, condition, reference plane, uncertainty, and applicability support each claim.

> **Before open volume**
>
> Step 1 requirements $\cdot$ Step 2 architecture $\cdot$ Step 3 bring-up $\cdot$ Step 4 characterization $\cdot$ Step 5 verify/validate $\cdot$ Step 6 qualification $\cdot$ Step 7 manufacturing/ATP $\cdot$ Step 8 pilot $\cdot$ Step 9--10 fleet control (Appendix D.18, Table 7.3).
...
**Key idea.** Product readiness is a sequence of decisions, not a catalog of tests. Begin by defining success and closing the architecture on paper. Establish interpretable hardware, build the behavioral model, verify requirements and validate intended use, qualify life, prove manufacturing control, and then compare the model with pilot and fleet reality. Each step should produce evidence for one explicit decision and identify the uncertainty that remains.

## The core IM/DD measurements

Once the readiness flow is clear, the measurement list is organized around isolation: transmitter, channel, and receiver. That split is older than PAM4. Long before TDECQ, field engineers learned that a dark link can be a dead laser, a dirty connector, or a dead TIA, and that guessing which one burns hours. Bisecting those three domains is still how you keep debug from turning into simultaneous retunes of everything.

### Transmitter

Start with the light leaving the faceplate or the CPO fiber array. For PAM4, the headline metric is *TDECQ* (transmitter and dispersion eye closure quaternary): a reference equalizer is applied to the captured eye and the residual penalty is reported in dB (§7.6). Alongside it you read *OMA* (outer), extinction ratio, and *RLM* (level linearity), plus wavelength, spectral width, and RIN with a bias-driver versus feedback bisect (§5.7, §5.8, §4.3.1).

What else you add depends on the transmitter style. Laser-bearing modules need LIV, threshold, slope, SMSR, and chirp checks for DMLs (§5.7, §5.4). External MZMs (TFLN or silicon) need EO $S_{21}$, $V_\pi$, quadrature bias versus temperature, and driver-path eye symmetry at baud (§3.14.3, §7.6). Microring banks need resonance alignment, thermal tuning, neighbor crosstalk, and peaking-network EO $S_{21}$ (§3.14.3, Chapter 6). The point of the list is not completeness for its own sake: it is knowing which instrument answers which hypothesis when the eye closes.
...
**Table 7.4.** Measurement mapping: metric, instrument, rationale, and failure signature in one reference. Row explanations follow; chapter subsections give the full treatment of each metric.
...
: the stressed-eye counterpart used on the receiver side under a named PMD, adding a calibrated stressor to test margin rather than transmitter quality alone. Distinguish SECQ from a general stressed-receiver sensitivity test. See §7.7.
...
A failing PAM4 link rarely announces which block is wrong. Ask the investigation question first, then pick the gear that can separate the leading hypotheses. Loopback topology tells you which side of the optical connector owns the fault (§7.4.2).
...
: (digital communication analyzer) for PAM4 eyes, TDECQ, OMA, RLM (§7.6). Needs a reference receiver filter matched to the PHY under test.
...
7.  **Quality snapshot.** On a Tx-capable path: OMA/RLM/TDECQ or module diagnostics that proxy them (§7.6). Record CMIS + BER + case $T$ together so later triage has a baseline (§7.14).

The numbered sequence above is primary. Table 7.5 is a wall-chart quick reference only; do not maintain a second conflicting procedure.
...
**Table 7.5.** Module bring-up checklist. LOS = loss of signal; LOL = loss of lock. Limits come from the ATP and PMD, not from this table.

##### Production-representative corners.

Bench corners ($T$, $V$) are necessary and not sufficient. Chassis thermal, host rails, and ORL belong before the program's DVT-phase exit on a representative unit. The full set in Table 7.6 belongs before PVT-phase exit (Table 9.2, §7.1).
...
**Table 7.6.** Production-representative corners. A quiet BERT at 25 $^\circ$C with pristine fiber is characterization, not production readiness.

### Reading the production-corner map

Quiet $T$/$V$ characterization maps the part. Table 7.6 is the checklist. Read it in four groups before you invent new corners.
...
Chassis thermal, host rails, and ORL are the minimum before the program's DVT-phase exit on a representative unit. The full set belongs before PVT-phase exit (Table 9.2, §7.1). Later fleet monitoring must not invent coverage these corners never ran.
...
- **Golden swap:** known-good module in the suspect host slot, then suspect module in a known-good slot. That single swap splits host vs. module before you open FA (§7.14).
...
Table 7.7 is the working map. Read left to right: observe, check telemetry, pick a provisional bucket, then run the named confirm measurement before you open an RMA or change a design rule.
...
**Table 7.7.** Fleet triage map: symptom to provisional bucket to confirm measurement. Perf $=$ performance (design/operating point); reliability $=$ time-dependent wear; mfg $=$ lot/process/install excursion. Row notes follow.
...
3.  **Classify** with Table 7.7. Write the bucket on the ticket before FA starts.
...
Product readiness is a chain of evidence, not a single pass: a number means nothing without its reference plane, its corner, and its method. The chapter's lifecycle, instruments, and triage tree are that evidence chain from bench to fleet.

### How it is measured

Use the least complex instrument that can falsify the current hypothesis. Table 7.4 maps every key metric to its instrument, rationale, and failure signature in one lookup; the bring-up sequence (§7.11) orders those instruments into a workflow.

### How it fails

Readiness evidence fails when the setup, sample, or acceptance rule does not match the product claim. Common misses are a stale calibration, the wrong reference plane, a golden host that hides interop risk, pristine fiber that hides ORL sensitivity, short BER dwell, one lane tested without neighbors, and chamber temperature used as a substitute for measured case temperature. These are test escapes even when the device physics is sound.
...
*Spoken answer.* "Characterization maps behavior and distributions. Verification checks a frozen requirement at a named plane. System validation asks whether the complete product works for intended system use. Reliability qualification bounds permanent degradation from life and environment. Manufacturing validation asks whether production can reproduce and control the design. Production acceptance testing, commonly ATP, dispositions a unit or population. Fleet monitoring asks whether the deployed population matches the release model. Same meters can serve several disciplines; the question and decision differ" (Table 7.2, Chapter 8, Chapter 9).
...
*Trap:* calling them "levels of testing" or interchangeable phase names.
...
*Spoken answer.* "Define requirements, review architecture, bring up hardware, characterize behavior, verify requirements and validate system use, qualify reliability, validate manufacturing, run a controlled pilot, ramp mass production, monitor the fleet, and feed learning into the next revision. Earlier steps make later evidence interpretable. Life and manufacturing evidence answer different residual uncertainties after the shipping envelope is clear. Calendar work can overlap, but a later pass cannot substitute for missing earlier evidence" (Table 7.3).

*Pressure follow-up.* "Where do EVT, DVT, and PVT fit?"\
*Answer pivot.* "They are program-phase labels, not evidence definitions. I would translate each gate into the hardware population, required evidence, open risks, and exit decision."

*Trap:* reciting Steps without saying why order matters, or treating EVT/DVT/PVT as universal engineering definitions.

##### Question 4. What happens during architecture review?

*Tests:* budget and derating priors.

*Spoken answer.* "I ask whether the design can plausibly meet requirements before hardware makes changes expensive. I close optical, noise, thermal, electrical, reliability, and manufacturing budgets on stated assumptions, and I name the thin margins that later readiness evidence must challenge. Architecture review is a path check, not life proof."
...
*Spoken answer.* "Bring-up establishes a known, reproducible operating state: identity, rails, firmware, CMIS, light, lock, alarms, basic power, and a simple link. I am not proving full margin yet. I am separating integration fails from product-performance questions" (§7.11).
...
*Spoken answer.* "Requirement and margin verification closes frozen specs at named planes with stated uncertainty. Intended-use validation asks whether production- representative hosts, peers, firmware, fiber plants, and workloads keep those boundaries intact. Same lifecycle step, different claims" (§7.3.7).
...
*Trap:* treating a verified requirement result as complete system validation.

##### Question 9. How would you run an optical-margin test?

*Tests:* waterfall and planes.

*Spoken answer.* "Trusted baseline at a named plane and BER condition, then stepped attenuation with a full BER waterfall, not only the fail point. Repeat at relevant $T$/$V$ corners and combine credible stresses. Report onset, boundary, uncertainty, and remaining margin without double-counting penalties" (§7.9).
...
*Spoken answer.* "A pilot is Step 8 operational readiness evidence. It tests whether lab and factory models survive deployment. Bounded serials, enhanced telemetry, exit criteria, and rollback. Compare BER, FEC, retrains, temperature, power, and cohort rates to the release model. It is not a small shipment" (§7.3.10).
...
*Trap:* "customers are happy," or calling the pilot "validation" of the entire lifecycle.

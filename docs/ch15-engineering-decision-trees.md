...
Same order as Table 10.2, not a second Staff loop. Do not treat a bring-up pass as margin evidence, or an HTOL pass as production readiness (§10.3, Appendix A.8.5, Appendix F.1.1).

*Legend.* Program phases: EVT, DVT, PVT, pilot, MP. Engineering disciplines: characterization, verification, system validation, reliability qualification, manufacturing validation, production control, fleet monitoring. Program phases group work. Engineering disciplines define the evidence (§10.4, Appendix F.18).

Step 6 (qualify reliability): mechanism $\rightarrow$ stress $\rightarrow$ observable $\rightarrow$ acceptance $\rightarrow$ confidence. See Chapter 10.

Step 7 (validate manufacturing): production reference $\rightarrow$ representative builds $\rightarrow$ trusted measurement $\rightarrow$ yield/ATP/SPC $\rightarrow$ ramp. See Chapter 10.
...
Scope sets severity and priors. Correlation after isolation unlocks contain, pause, replace, or supplier escalate (§11.10, Appendix G.12).
...
Use provisional containment when the population can grow, then refine the hold after scope analysis. The system owner keeps responsibility for evidence quality and verifying corrective action (Appendix G.18, §11.10, Appendix D.16).
...
*Route.* Measurement-system and station-correlation (Chapter 10, Appendix D.14).\
...
*Route.* Package-fatigue and intermittent-mechanism (Chapter 10, Appendix D.3).\
...
*Route.* Manufacturing-escape versus reliability-gap (Chapter 10, Appendix D.9).\

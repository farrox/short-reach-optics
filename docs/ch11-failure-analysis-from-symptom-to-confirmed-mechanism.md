---
layout: default
title: "Ch 11: Failure Analysis: From Symptom to Confirmed Mechanism"
---

# 11 Failure Analysis: From Symptom to Confirmed Mechanism

Chapter 10 detects and bounds the operational anomaly: FEC activity, retrains, cohorts, severity, and initial containment (Chapter 10). This chapter begins when the issue becomes an investigation that must confirm mechanism and ownership. Earlier chapters own the underlying physics (Chapter 4, Chapter 5, Chapter 6). Symptom recipes and the fleet-bucket map live in Appendix I.
...
: Investigation remains open. Keep an owner, containment, next experiment, and review date. Do not treat unknown mechanism as a completed root cause.
 ...
 The checklist in Table 11.1 is a lifecycle, not a suggestion list. Each step removes a class of uncertainty before the next step spends lab time.
...
**Table 11.1.** Failure-analysis checklist. Fill these fields as you walk the sequence.
...
Containment should match the evidence. Drain one link, hold a lot, pause one revision, or increase telemetry while the mechanism remains open. Do not wait for perfect certainty when exposure can continue growing, and do not stop an unrelated fleet without evidence (§10.8, Chapter 10).
...
A swap localizes ownership. It may not prove the microscopic mechanism. Do not treat the last surviving hypothesis as proof (§11.1.1).
...
Do not choose the corrective action until the mechanism and ownership boundary are sufficiently supported. If confirmed, possible controls include design change, supplier process, incoming inspection, ATP, sample audit, SPC, telemetry, service procedure, or qualification (Appendix D.3, §7.4.11).
...
Operational detection and cohort bounding belong to Chapter 10. For mechanism confirmation: preserve state before reseat; compare affected and unaffected cohorts; run reversible module, fiber, and host-port swaps; correlate thermal, ORL, and supply events; hand destructive confirmation to the reference routes when needed (Appendix I.8, §10.11).

## Fleet handoff and reference routes

Lab debug asks what is broken on this unit. Fleet triage asks which bucket owns the fix. After operational detection in Chapter 10, use the fleet map in Table I.1, Appendix I.14 to assign performance, reliability, manufacturability, plant, or host ownership. Contain the population and clear the measurement system before opening supplier FA.
...
*Spoken answer.* "I ask whether the symptom is one lane, module, host, fiber, lot, site, firmware, or a broader cohort. I compare affected and unaffected denominators. Containment matches that width: drain a link, hold a lot, or raise telemetry. I do not wait for perfect certainty when the population can grow" (§11.3, §10.8).
...
*Spoken answer.* "I preserve field evidence, scope the cohort, and ask whether this is wear-out, an infant escape, plant practice, or a detection gap. Component FIT is not fleet availability. I confirm mechanism before changing life models or ATP" (§10.9.1, §11.1).

...
: Named-plane waterfall first; classify shift versus floor, then power, eye, noise, timing, and spectrum (§10.2, §10.2.1).

Eye closure or low ER

: Split bandwidth, drive, bias, reflection, dispersion, and resonance alignment (§10.6, §10.3).

Lane imbalance

: Split source, modulator or ring, filter or MUX, fiber attach, driver, and receiver (§10.4).

Temperature sensitivity

: Track power, wavelength, lock error, TEC or heater headroom, receiver margin, and package stress (§10.12).

Wavelength drift

: Split source movement from filter, ring, TEC, and control-loop movement (§10.5).

Intermittent bursts

: Preserve counters before reseating. Check connector contamination, ORL, supply noise, lock state, and weak attach (§10.8).

Yield drop

: Clear the tester, then split lot, site, process step, assembly, firmware, and calibration (§10.10).
...
4.  Preserve telemetry, then inspect connectors before cleaning (§10.9). Measure insertion loss and ORL. Use a golden fiber and module swap to separate field plant from module.
...
Waterfall (BER vs power)
  |-- power not held --&gt; power path (§10.1)
...
3.  If the curve floors, continue in §10.2.1 and split intrinsic RIN, electrical noise, ORL, MPI, and crosstalk.
...
Restore the changed process or measurement input, add a statistical control at the first observable point, revise ATP only with correlation data, and require a new first-article check before releasing volume (§8.10). *Recurrence control:* SPC at the first separating observable; FAIR after change.

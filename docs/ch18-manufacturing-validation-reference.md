...
Program-phase labels (EVT, DVT, PVT, MP) are owned in §10.4, Table 10.3. They are not a second lifecycle. The table below is a manufacturing-evidence lookup: which release call the gate evidence supports when the program uses those names.
...
**Table G.1.** NPI gates as a manufacturing evidence lookup. Pilot sits between PVT and MP. MP is sustained control, not fleet monitoring alone. Phase definitions: §10.4. Argument structure: Chapter 10.
...
- Qualification re-entry depth when life or environmental mechanisms are affected (Chapter 10, Appendix F).

- Incoming sample plan and SPC metrics with owners and reaction plans.

- ECO and firmware version control matching the production reference freeze in Chapter 10.
...
The following sections are the former manufacturing-validation chapter body, kept as reference depth. Start from Chapter 10 for the interview spine. Extended drills remain in Appendix G.20.1, Appendix M.11.
...
*Check.* Only when evidence ties the mechanism to a preventable production or test-control gap. Otherwise wear-out, interop, install, service, software, or residual latent risk (Appendix G.18, Chapter 11).

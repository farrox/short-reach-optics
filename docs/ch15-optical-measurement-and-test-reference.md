---
layout: default
title: "Ch 15: Optical Measurement and Test Reference"
---

# 15 Optical Measurement and Test Reference

This appendix is a lookup reference for measurement selection, metric conditions, instruments, stressed-receiver methods, link-budget accounting, and CMIS diagnostics. It does not define the product-readiness lifecycle; that story lives in Chapter 9.
...
: OSA or wavemeter for wavelength, SMSR, and side modes (Chapter 7).

Did receiver margin shift or floor?

: BERT with calibrated attenuator or stressor; FEC counters for shape (§6.4, Appendix E.4, Appendix H.19).

Is intensity noise the floor?

: PD plus ESA or a dedicated RIN analyzer under a defined ORL (§6.3.1).
...
Insertion loss, ORL, dispersion, and filtering belong with the plant assumptions used in the link budget. Reflections can seed feedback noise, MPI, distortion, and power-independent floors even when average power is stable (§6.3.1, Chapter 7).
...
TDECQ evaluates transmitter quality through a reference-receiver model. SECQ is not the same test as a generic stressed-receiver sensitivity sweep, even when both use attenuation and ISI. Always state the PMD, FEC architecture, error model, metric, stress calibration, and test duration. For LPO, stressed Rx margin on the host-side receiver is as important as transmitter-quality metrics (Chapter 5, Appendix H.5.1).

## Link-budget accounting reference

A link budget is a signed ledger from transmitter to receiver. For IM/DD short reach, start from outer OMA at a named Tx plane, subtract each loss once, and compare the remainder to receiver sensitivity at the named BER or FEC objective (§3.12, §6.4).
...
Keep power, signal-quality, timing, thermal, and control authority as separate ledgers when the impairment is not a pure optical-power number (§7.19, Appendix D.10). Distinguish design allocation from measured net margin across the operating envelope.
...
Operational use of management state is decision-oriented, not a register inventory (Chapter 9, Appendix H.22):
...
*Check.* Optical reference plane, BER or FEC condition, pattern, temperature, wavelength, and stress method when claimed (Chapter 9, §6.4).
...
<div class="nav-links">
  ...
  <a href="ch16-reliability-qualification-reference">Next &rarr;</a>
</div>

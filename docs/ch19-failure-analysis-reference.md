...
This appendix is a symptom-route lookup for failure analysis. The investigation method (preserve, scope, falsify, confirm, correct, prevent) lives in Chapter 10. Use these routes after the population is scoped and the failing state is preserved. Do not treat a recipe as a substitute for the canonical sequence.
...
Lab debug asks: *what is broken on this unit?* Fleet triage asks: *which bucket does this failure belong in, and who owns the fix?* The investigation method lives in Chapter 10. Optical programs at fleet scale own that bucket split across performance, reliability, and manufacturability. Wrong bucket wastes weeks.
...
<div class="nav-links">
  <a href="ch18-ai-fabric-context">&larr; Previous</a>

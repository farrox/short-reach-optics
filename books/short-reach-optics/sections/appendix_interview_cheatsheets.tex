...
2.5D versus 3D/CPO, and fat-tree/HPC sheets, then models through WDM,
productization, and failure analysis. The productization flashcards at the end
are a secondary reference bank, not the primary study set. Full depth:
\Cref{ch:multilevel,ch:equalization,ch:models,ch:lasers,ch:wdm,ch:packaging,ch:hpc,ch:productization,ch:failure-modes}.
...
\clearpage
\section{Fat-tree and HPC rack}
\label{sec:cheat-hpc}

\textit{Goal.} Size topology from collectives and failure domains, not from a
vendor rack photo.

\paragraph{Mental checklist.}
Workload / collective $\rightarrow$ Scale-up vs scale-out $\rightarrow$
Bisection $\rightarrow$ Oversubscription at named tier $\rightarrow$ Network
rails $\rightarrow$ Ports $\rightarrow$ Optical endpoints $\rightarrow$
Lanes/$\lambda$ $\rightarrow$ PAM/baud $\rightarrow$ EQ/FEC owner
$\rightarrow$ Placement $\rightarrow$ Lasers/watts $\rightarrow$ Replaceables
$\rightarrow$ Power / cooling / service

\paragraph{What I need to know.}
\begin{itemize}
\item Fat tree is folded Clos: leaves, spines, path diversity.
\item Bisection is the worst equal cut, not average utilization.
\item Oversubscription needs a tier and a traffic pattern.
\item Network rails can buy isolation; state shared switches, trunks, control,
      power, and software.
\item Scale-up and scale-out are different planes.
\item A rack sketch must count ports, optical endpoints, modules, and
      replaceables (\Cref{sec:hpc-rack-to-phy}).
\end{itemize}
...
\paragraph{60-second answer.}
``I start from the collective and failure domain. I split scale-up from
scale-out, size bisection and oversubscription at each tier, choose rail
count, sketch the rack with ports and optics, pick copper versus optics by
reach, and I state power and serviceability. Topology names come after those
numbers.''

\paragraph{Pressure.}
Pressure: ``Average utilization is only 20\%. Why not 4:1 oversub?''\\
Pivot: ``Averages miss synchronized bursts. I need the collective-period peak
at that tier.''

\footnotesize Depth:
\Cref{ch:hpc,sec:hpc-rack-to-phy,sec:topologies,sec:collectives,sec:scale-up-out}.

\clearpage
\section{Chapter 11: Productization and fleet readiness}
...
\clearpage
\section{Chapter 12: Failure analysis}

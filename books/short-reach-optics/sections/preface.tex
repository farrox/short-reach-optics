...
signaling (including multilevel PAM4/PAM8 design), optical devices, wavelength
control, packaging and fabric context, and a compact productization path from
requirements to controlled ramp. Failure analysis closes the loop
(\Cref{ch:failure-modes}). Detailed qualification, manufacturing, and operations
catalogs live in the appendices. Fabric survey context lives in
...
\item[Deep learning] Read the body chapters from requirements downward
      ...
      signaling in \Cref{ch:multilevel}), then the failure-analysis method
      (\Cref{ch:failure-modes}). Every major chapter asks: How does it work? How
      is it measured, and what uncertainty does the measurement remove? How does
      it fail? How is it debugged?

...
signaling, optical devices, wavelength control, packaging and fabric context, and
a compact productization path from requirements to controlled ramp. Failure
analysis closes the loop (\Cref{ch:failure-modes}). Detailed qualification,
manufacturing, and operations catalogs live in the appendices. Fabric survey
context lives in \Cref{app:ai-fabric-context}.
...
\item[Deep learning] Read the body chapters from requirements downward
      (\Cref{ch:role} through \Cref{ch:productization}), then the failure-analysis
      method (\Cref{ch:failure-modes}). Every major chapter asks: How does it

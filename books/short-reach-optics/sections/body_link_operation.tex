% Relocated narrative from ch08_ai_datacenter_networking.tex. Chapter aliases live on ch_productization.tex. Do not re-add \chapter or ch:* labels here.
...
\begin{description}
\item[Pre-FEC error activity]
      \firstterm{preFEC} indicates physical-link burden before correction. The
      counter may record bits, symbols, codewords, or implementation-specific
      events, so its definition and accumulation window must be stated.
\item[Corrected errors]
      \firstterm{correctedErrors} indicate FEC activity and consumed margin.
      Rising correction demand can reveal shrinking physical margin even while
      delivered traffic remains error-free. It is an early-warning signal, not
      by itself proof of an imminent outage.
...
\item[Post-FEC residuals]
      \firstterm{postFEC} residuals describe what remains after correction. Zero
      observed residuals do not prove unlimited margin.
\end{description}
...
The productization chapter owns the readiness lifecycle and pilot sequencing

\chapter{AI Fabric Context}
\label{app:ai-fabric-context}

This appendix holds the fabric survey that places optics in scale-up and
scale-out networks: topologies, module styles, electrical reach, collectives,
circuit switching, co-packaging, latency, power, and cost. Use it when you need
system context. The compact productization chapter owns pilot and ramp judgment
(\Cref{ch:productization,ch:networking}). The relocated link-operation narrative
is at the end of this appendix.

\section{Scale-up versus scale-out}
\label{sec:scale-up-out}

Design-level rack arithmetic, fat-tree judgment, network rails, and
oversubscription live in \Cref{ch:hpc}. This section keeps the fabric
vocabulary survey.
...
Whiteboard topology design, bisection, and the worked rack exercise live in
\Cref{ch:hpc}. This section keeps the fleet-scale survey.
...
rail-optimized, or dragonfly layouts. In a conventional $k$-ary three-stage fat
tree, both host count and physical link count scale as $O(k^3)$; the pressure
is ports, cable ends, and optical endpoints per host, not a different
asymptotic exponent (\Cref{ch:hpc,sec:hpc-fattree-cost}). Network-rail designs
(corresponding NIC ports across nodes, with stated shared-risk boundaries) rose
with collective-heavy training and inference because they can cut
...
Integration-style judgment (2.5D versus 3D, CPO is not automatically 3D) is in
\Cref{ch:packaging}.
...
Rack-level bisection, oversubscription, and network-rail design for collectives
live in \Cref{ch:hpc}. This section keeps the workload vocabulary and optics
mapping.
...
Optical engineering maps to these patterns indirectly. More network rails and
higher per-lane rate cut time spent in collectives; CPO/XPO raise faceplate
bandwidth so fewer hops are needed
(\Cref{sec:cpo-status,sec:xpo,sec:topologies,ch:hpc}). Protocol choice (UEC vs
IB) sets lossless delivery and congestion behavior
...
Design-level judgment for 2.5D versus 3D, thermal partitioning, known-good
die, fiber attach, and serviceability lives in \Cref{ch:packaging}. This
section keeps the shipping-program survey.
...
\section{Relocated link-operation narrative}
\label{sec:relocated-link-operation-narrative}

The following sections are the former optical-links-in-operation chapter body,
kept as reference depth. Pilot and ramp judgment for interviews lives in
\Cref{ch:productization}. Extended drills remain in
\Cref{sec:networking-interview-qa,sec:cheat-flashcards}.

\input{sections/body_link_operation}

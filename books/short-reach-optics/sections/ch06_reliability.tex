...
\textit{Spoken answer.}
``FIT and DPPM use different denominators. FIT is an exposure-normalized
failure rate, one failure per $10^{9}$ device-hours, so I name the population
unit, failure definition, exposure, confidence, hazard assumption, and whether
the value is observed or predicted. DPPM is a unit-normalized quality rate at a
named boundary, so I state incoming, outgoing, or escaped, plus inspection
coverage and how rework, retest, and duplicate returns are counted. I do not
trade one for the other. Qualification sample size follows from the claimed
rate or degradation, stress hours, acceleration, confidence, and population
diversity. Zero failures do not establish a zero rate; they establish an upper
bound under stated assumptions''

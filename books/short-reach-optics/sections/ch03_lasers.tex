...
\item[DFB (distributed feedback)] a grating along the active region gives
      single-mode output; the workhorse continuous-wave (CW) or directly
      modulated source for CWDM and LAN-WDM (\Cref{sec:dfb-eml}).
\item[DBR (distributed Bragg reflector)] the grating sits outside the gain
      region; enables tunable variants.
\item[DML (directly modulated laser)] modulate the bias current directly: cheap
...
\item[VCSEL] 850--940~nm multimode sources for short-reach links over multimode
...
started with an InP laser chip. Two configurations still dominate production:
...
parts add a TEC and lock to a grid.
...
\item COD (catastrophic optical damage) at the facet under overstress.
\end{itemize}
...
accelerated stress (HTOL, temperature cycle, damp heat) and projects field life

...
evidence. Debugging is Bayesian inference in the lab with engineering priors
(common modes first). Chunk the spine as Understand / Investigate / Resolve /
Prevent. Every answer ends with a decision. Name the ledger that moved.}
{Quiz me on the three principles, priors, and the four spine phases. Give me a
...
\noindent\textit{Debug.} Bayesian inference in the lab. Priors favor common
modes. Minimum measurement that kills the most hypotheses.\par
\noindent\textit{Spine (4 phases).} Understand $\rightarrow$ Investigate
$\rightarrow$ Resolve $\rightarrow$ Prevent (end on the decision).\par
\noindent\textit{Checklist.} Scope? Ledger? Power or quality? Fastest
measurement? Decision? Control?\par
\noindent\textit{Memorize.} Three principles, four-phase spine, five ledgers,
power-versus-quality fork, validation ladder, decision vocabulary, two
stories. Rest is reference.\par
\noindent\textit{Five ledgers.} Power $\cdot$ Noise $\cdot$ Timing $\cdot$
Spectral $\cdot$ Control.\par
\noindent\textit{Power fork / scope / ladder.} Apply the fork; first scope
(unit$\rightarrow$lot$\rightarrow$vendor$\rightarrow$fleet); walk the
validation ladder (\Cref{sec:interview-validation-ladder}).\par
...
\noindent\textit{Waterfall / thermal.} Shift vs floor vs burst. Cool-down
recovery vs monotonic aging.\par
\noindent\textit{Lab time / hierarchy.} Golden swap $\rightarrow$ power
$\rightarrow$ bias sweep $\rightarrow$ DCA/OSA/RIN. Telemetry $\rightarrow$
bench $\rightarrow$ swap $\rightarrow$ characterization $\rightarrow$ FA/DPA.\par
\noindent\textit{Traps / ownership / delivery.} No component-first; no
symptom-as-root-cause; no stop after fix. Contain, notify, FA, ATP, monitor
RMA. Memorize green 30-second boxes only.\par
}

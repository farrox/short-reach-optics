...
the green boxes in \Cref{app:interview-frameworks}, then the wall-chart trees in
\Cref{app:decision-trees}. Do not re-read every worked answer unless a topic is
weak. Compression is the point: recover the method in minutes, not hours.
...
Measurements unlock actions. Name the action, the evidence that unlocks it,
and the residual risk if you choose it too early.

\paragraph{Ship / don't ship.}

\textbf{Purpose.}
Does the population meet ATP and the life model for the claimed envelope?

\textbf{Uncertainty removed.}
Hero samples and open mechanisms do not answer ship. Multi-lot ATP and named
life evidence do.

\textbf{Exit criteria.}
\textbf{Exit when} population data meet versioned ATP and life claims, or
explicitly fail them.

\textbf{Decision unlocked.}
Ship, hold, or restrict the envelope.

\textbf{Risk if chosen too early.}
You learn the escape mechanism from customer outage.

\paragraph{Continue validation.}

\textbf{Purpose.}
Does remaining uncertainty still block a ship or architecture call?

\textbf{Exit criteria.}
\textbf{Exit when} the missing measurement or corner is named and scheduled.

\textbf{Decision unlocked.}
Spend lab time on a specific ladder stage, not on open-ended ``more testing.''

\textbf{Risk if chosen too late.}
You keep measuring after the decision is already blocked only by courage.

\paragraph{Escalate supplier.}

\textbf{Purpose.}
Does scope show a lot, site, or vendor signature that the supplier must own?

\textbf{Exit criteria.}
\textbf{Exit when} correlation plots and split RMA codes justify the
escalation pack.

\textbf{Decision unlocked.}
Open 8D / CAPA with evidence; do not escalate a single dirty connector as a
vendor crisis.

\textbf{Risk if chosen too early.}
You burn supplier trust and miss a host or plant cause.

\paragraph{Derate.}

\textbf{Purpose.}
Is margin too thin for the full claim, but acceptable under tighter use?

\textbf{Exit criteria.}
\textbf{Exit when} the restricted envelope is written and validated at the
new corner.

\textbf{Decision unlocked.}
Ship under derate, or reject if derate kills the product purpose.

\textbf{Risk if chosen too early.}
You hide a design defect behind a temporary thermal or reach cut.

\paragraph{Second source.}

\textbf{Purpose.}
Does single-vendor risk exceed fleet tolerance even if the first source
still works?

\textbf{Exit criteria.}
\textbf{Exit when} business risk and dual-qual cost are explicit, and the
second source has its own ATP and life evidence.

\textbf{Decision unlocked.}
Fund dual source, or accept single-source risk in writing.

\textbf{Risk if chosen too early.}
You dual-source before the first source's failure mode is understood, and
copy the same blind spot.

\paragraph{Contain lot.}

\textbf{Purpose.}
Is there a date-code or lot escape that must stop further exposure today?

\textbf{Exit criteria.}
\textbf{Exit when} suspect lots are identified and ship/fleet holds are in
place.

\textbf{Decision unlocked.}
Quarantine WIP and field exposure while FA continues.

\textbf{Risk if chosen too late.}
Bad lots keep shipping while SEM photos are pending.

\paragraph{Modify ATP.}

\textbf{Purpose.}
Was an escape path found that production must catch next time?

\textbf{Exit criteria.}
\textbf{Exit when} the new or tightened ATP row has limits, guardband, and
GR\&R.

\textbf{Decision unlocked.}
Version the ATP with the requirements slice; hold lots that fail the new
screen.

\textbf{Risk if skipped.}
FA closes; the factory ships the same escape tomorrow.

\paragraph{Open RMA / request FA.}

\textbf{Purpose.}
Does a field or partner unit need mechanism work beyond remote triage?

\textbf{Exit criteria.}
\textbf{Exit when} preserve pack exists and remote triage cannot close
ownership.

\textbf{Decision unlocked.}
Pull the unit under chain of custody; do not reseat away the evidence first.

\textbf{Risk if chosen too early.}
You RMA healthy units and starve the lab with noise.

\paragraph{Perform DPA.}

\textbf{Purpose.}
Do you need physical confirmation of facet, solder, FAU, or die?

\textbf{Exit criteria.}
\textbf{Exit when} non-destructive evidence has narrowed the site and DPA
will change the decision.

\textbf{Decision unlocked.}
Section the part; update mechanism and CAPA.

\textbf{Risk if chosen too early.}
You destroy the only unit before electrical and optical falsifiers run
(\Cref{sec:failure-analysis-checklist}).

\paragraph{Change firmware.}

\textbf{Purpose.}
Is the control loop, table, or guard wrong while the hardware is healthy?

\textbf{Exit criteria.}
\textbf{Exit when} before/after at the failing corner shows the firmware
change restores margin.

\textbf{Decision unlocked.}
Rev firmware under change control; treat the rev like a process change for
FAIR.

\textbf{Risk if chosen too early.}
You mask a hardware wear-out with a table patch that drifts again.

\paragraph{Retune calibration.}

\textbf{Purpose.}
Is the device healthy but the setpoint or table segment wrong?

\textbf{Exit criteria.}
\textbf{Exit when} calibration restore clears the corner and LIV/SMSR/RIN
baselines are unchanged.

\textbf{Decision unlocked.}
Fix tables and screening; do not open laser life CAPA for a bad segment.

\textbf{Risk if chosen too early.}
You ``calibrate away'' real aging and ship drifting parts.

\paragraph{Monitor only.}

\textbf{Purpose.}
Is the rate tiny, flat, and without customer impact so watching is enough?

\textbf{Exit criteria.}
\textbf{Exit when} rate, trend, and impact criteria for monitor-only are
met and alarms are armed.

\textbf{Decision unlocked.}
Watch weekly; escalate to contain if growth or impact appears.

\textbf{Risk if chosen too early.}
A growing supplier-specific rate is treated as noise until it is a fleet
event.

\subsection{Why these actions are not interchangeable}

Ship and don't-ship need population and life evidence. Continue validation is
for named missing corners, not delay. Escalate supplier and contain lot need
scope. Derate and second source are product decisions, not FA hobbies. Modify
ATP is how FA becomes production. RMA, FA, and DPA climb the measurement
hierarchy only as far as the decision requires. Firmware and calibration are
allowed only when hardware baselines clear. Monitor-only is an economics call
with tripwires, not a default.
...
\execanswer{Validation is staged uncertainty reduction along the canonical
lifecycle: requirements, bring-up, nominal and margin characterization,
interoperability, environmental and reliability qualification, manufacturing
and ATP readiness, controlled pilot, then fleet deployment and monitoring.
Each stage answers a question the previous stage could not. Therefore I would
walk that order and refuse any test that answers no new question.}
...
cool-down recovers. At the failing temperature, read externally visible
remaining margin (BER/FEC, telemetry, control headroom). With engineering
access, add bias sweep, OSA, and external optical eye
(\Cref{sec:tree-blackbox-access}). Therefore I would fix the table or thermal
...
the device remains substantially healthy. Start black-box; recalibration
recovery updates probability but is not proof. With engineering access,
compare external baselines to ship data. Therefore I would route aging to
life/derate/replace and drift to table version control plus an ATP
loaded-corner check.}
...
A fuller glossary is in \Cref{app:abbreviations}. By the end of the week, know these
...
characterize margin. The job is the best decision with today's evidence.\par

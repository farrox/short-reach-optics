...
\textit{Learn:} characterization, margin, interoperability, qualification
(\Cref{sec:interview-validation-ladder,fw:validation-plan}).\\
\textit{Output:} Design a validation plan for a new optical module in two
minutes.

\paragraph{Day 4: Failure analysis.}
\textit{Learn:} decision trees, fleet scope, supplier escapes
(\Cref{app:decision-trees,fw:fleet,fw:supplier-escape}). Work
\Cref{sec:case-fleet-ber,sec:case-hot-ber} aloud; score with
\Cref{sec:interview-scoring-rubric}.\\
\textit{Output:} Debug an ambiguous failure without naming a component first.

\paragraph{Day 5: Manufacturing.}
\textit{Learn:} ATP, SPC, qualification, reliability
(\Cref{ch:reliability,fw:qual-plan,fw:atp-update}).\\
\textit{Output:} Discuss how you would take a link to volume production.

\paragraph{Day 6: Architecture.}
\textit{Learn:} IM/DD, WDM, AI networking, optical roadmap
(\Cref{ch:imdd,ch:wdm,ch:networking,ch:role}). Rehearse two true stories
(\Cref{sec:interview-story-template}).\\
\textit{Output:} Defend an architecture tradeoff with requirements first.
...
Compress: Day~1, Day~4 cases, Day~3 validation, Day~5 manufacturing, Day~7
mock. Keep the cheat sheets and Top~25. Cut new chapter reading first.

...
\Cref{sec:validation-workflow,tab:ladder}. Memorize this same order:

\begin{description}
\item[Requirements] What must be true?
\item[Architecture] Can this design meet requirements?
\item[Bring-up] Does the hardware fundamentally work?
...
\item[Controlled pilot] Do lab assumptions hold in a bounded cohort?
\item[Mass production] Can we sustain volume with control?
\item[Fleet monitoring] Does it remain healthy in operations?
\item[Feedback] What must change next?
\end{description}

Night-before path: Framework~7 in \Cref{app:interview-frameworks}.
...
\item Confirmed mechanism: the mechanism that survived, with evidence.
...
\item Recurrence control and measured result: the test, alarm, or process
      change, and the metric that confirmed it worked.
\end{enumerate}
...
\paragraph{Day 4: instruments, waterfall, debug forks, and case practice.}
...
Work \Cref{sec:case-fleet-ber,sec:case-hot-ber} aloud under time pressure and
score with \Cref{sec:interview-scoring-rubric}. Use the ten-concepts LLM practice box.
...
\subsection{The five questions behind every interview problem}
\label{sec:interview-five-questions}

\begin{enumerate}
\item What is the requirement?
\item What is the failure scope?
\item What mechanisms could explain it?
\item What measurement separates them (at which access level and plane)?
\item What decision does the evidence enable, and what control prevents recurrence?
\end{enumerate}

{\small\setlength{\parskip}{0.3em}%
\noindent\textit{Central loop.} Requirement $\rightarrow$ risk $\rightarrow$
mechanism $\rightarrow$ measurement $\rightarrow$ evidence $\rightarrow$
decision $\rightarrow$ control $\rightarrow$ learning.\par
\noindent\textit{Access levels.} Telemetry $\rightarrow$ system experiments
$\rightarrow$ external module $\rightarrow$ engineering breakout $\rightarrow$
destructive FA (\Cref{sec:interview-access-levels}).\par
\noindent\textit{Evidence language.} Observation / correlation / hypothesis /
confirmation / decision. Do not say ``data proves'' until confirmation exists.
\par
\noindent\textit{Lifecycle.} Requirements $\rightarrow$ architecture
$\rightarrow$ bring-up $\rightarrow$ characterization $\rightarrow$ margin
$\rightarrow$ interop $\rightarrow$ qualification $\rightarrow$ manufacturing
$\rightarrow$ pilot $\rightarrow$ MP $\rightarrow$ fleet $\rightarrow$ feedback
(\Cref{tab:ladder}).\par
\noindent\textit{Night before.} Open the matching playbook in
\Cref{app:interview-frameworks}. Memorize ``30-second answer'' callouts only.
\par
...
firmware, retune calibration, monitor only (\Cref{tab:interview-decisions}).
\par
}

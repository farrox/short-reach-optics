...
nodes:
\begin{description}
\item[Understand] Requirements, architecture.
\item[Investigate] Measurements, observations, hypotheses, isolation.
\item[Resolve] Root cause, corrective action.
\item[Prevent] Recurrence control, and the product decision that closes the
      answer.
\end{description}
...
\llmpractice{Three principles plus: the job is the best decision with today's
evidence. Debugging is Bayesian inference in the lab. Chunk the spine as
Understand / Investigate / Resolve / Prevent. Every answer ends with a
decision. Name the ledger that moved.}
{Quiz me on the three principles and the four spine phases. Give me a failed
module at high temperature with stable average power. Stop me if I skip scope,
the power fork, the control ledger, the product decision, or recurrence
control.}
...
with a decision and a control. Do not recite; adapt the skeleton to the
follow-up questions.

\paragraph{Memorize the 30-second versions.}
Nobody remembers six pages in an interview. Memorize the executive close for
each question. Deliver that first. Expand only where the interviewer asks:
\[
\text{30-second answer}
\longrightarrow \text{interviewer asks}
\longrightarrow \text{expand only there}.
\]
The 3-minute and 10-minute versions are references, not scripts.

\paragraph{Three lengths for every long answer.}
\begin{description}
\item[Executive (30 seconds)] Frame, decision, and one control. Memorize this.
\item[Engineering (3 minutes)] Scope, fork or ladder, key measurements, ledger,
      root-cause class, recurrence control.
...
\paragraph{Day 1: principles, four-phase spine, traps, and Staff judgment.}
Read the three principles, the answer spine, the traps, and
\Cref{sec:interview-under-uncertainty}. Memorize: every answer ends with a
decision; the job is the best decision with today's evidence; debugging is
Bayesian inference in the lab. Memorize the four phases (Understand /
Investigate / Resolve / Prevent), the six-question checklist, the decision
table, and the five ledgers. Speak one paragraph per phase. Use the
answer-spine LLM practice box. Write ownership language: stop ship, notify
supplier, request FA, update ATP, monitor RMA.
...
unlock decisions. Every measurement updates belief. The job is the best
decision with today's evidence, not finding the truth.\par
\noindent\textit{Debug.} Bayesian inference in the lab. Minimum measurement
that kills the most hypotheses.\par
\noindent\textit{Spine (4 phases).} Understand (requirements, architecture)
$\rightarrow$ Investigate (measure, observe, hypothesize, isolate)
$\rightarrow$ Resolve (root cause, corrective action) $\rightarrow$ Prevent
(recurrence control, decision).\par
...
Spectral $\cdot$ Control. Prefer ``control ledger exhausted'' over ``TEC maxed.''\par
\noindent\textit{Power fork.} Power moved: optical path. Power held: signal
quality or receiver.\par
...
\noindent\textit{Scope.} Unit $\rightarrow$ lot $\rightarrow$ vendor
$\rightarrow$ rack $\rightarrow$ datacenter $\rightarrow$ fleet.\par
\noindent\textit{Waterfall.} Shift vs floor vs burst.\par
\noindent\textit{Thermal vs aging.} Recovers on cool-down vs monotonic
permanent.\par
\noindent\textit{Lab time.} Golden swap $\rightarrow$ power $\rightarrow$ bias
sweep $\rightarrow$ then DCA/OSA/RIN.\par
\noindent\textit{Measure hierarchy.} Telemetry $\rightarrow$ bench
$\rightarrow$ swap $\rightarrow$ characterization $\rightarrow$ FA/DPA.\par
\noindent\textit{Traps.} No component-first. No symptom-as-root-cause. No stop
after fix. No instrument list without decisions. No one-unit-as-fleet.\par
\noindent\textit{Ownership.} Contain, notify, FA path, ATP/telemetry control,
monitor RMA, requalify.\par
\noindent\textit{Delivery.} Memorize 30-second closes. Expand only where asked.\par
}

...
\paragraph{Day 1: Mental models.}
\textit{Learn:} five ledgers (\Cref{sec:interview-margin-ledgers}), validation
lifecycle (\Cref{tab:ladder}), margin thinking, reference planes, Staff pattern
(\Cref{sec:interview-staff-pattern}).\\
\textit{Output:} Explain aloud, ``How would you debug a BER failure?''

\paragraph{Day 2: Optical fundamentals.}
\textit{Learn:} power, OMA, ER, RIN, BER, sensitivity
(\Cref{ch:imdd,sec:interview-waterfall}).\\
\textit{Output:} Explain aloud, ``Why does power not equal quality?''

\paragraph{Day 3: Validation.}
\textit{Learn:} characterization, margin, interoperability, and where
qualification and manufacturing sit in the lifecycle
(\Cref{sec:interview-validation-ladder,fw:validation-plan,tab:ladder}).\\
\textit{Output:} Design a validation plan for a new optical module in two
minutes.

\paragraph{Day 4: Reliability qualification.}
\textit{Learn:} mechanism-driven qualification, HTOL versus burn-in, sample
confidence (\Cref{ch:reliability,fw:qual-plan,sec:tree-qual-evidence}).\\
\textit{Output:} Walk a laser-degradation qualification argument aloud.

\paragraph{Day 5: Manufacturing.}
\textit{Learn:} measurement systems, ATP, yield, SPC, and scale-up
(\Cref{ch:manufacturing,fw:atp-update,sec:gauge-rr,tab:npi}).\\
\textit{Output:} Discuss how you would take a link to volume production.

\paragraph{Day 6: Architecture and failure analysis.}
\textit{Learn:} multilevel signaling, equalization, packaging, and HPC rack
architecture
(\Cref{ch:multilevel,ch:equalization,ch:packaging,ch:hpc}); IM/DD and WDM as
needed (\Cref{ch:imdd,ch:wdm}); fleet trees and cases
(\Cref{app:decision-trees,fw:fleet,sec:case-fleet-ber}). Rehearse two true
stories (\Cref{sec:interview-story-template}).\\
\textit{Output:} Defend an architecture tradeoff; debug one fleet case without
naming a component first.
...
Compress: Day~2 fundamentals, Day~6 architecture, Day~1 mental models, Day~7
mock. Keep the cheat sheets and Top~25. Add Day~3--5 readiness only if the role
is lifecycle-led. Cut new chapter reading first.

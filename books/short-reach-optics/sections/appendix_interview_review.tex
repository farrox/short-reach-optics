...
Qualification =
bring-up~$\rightarrow$~margin~$\rightarrow$~environment~$\rightarrow$~interop~$\rightarrow$~reliability~$\rightarrow$~ATP~$\rightarrow$~pilot~$\rightarrow$~fleet.
...
sections only say ``walk the validation ladder''; this is the canonical map.
\Cref{tab:ladder,tab:ladder-gates} are grouped views of the same order.

\begin{description}
\item[Requirements] Freeze operating envelope, hosts, reach, BER/FEC, optical
      and electrical limits, environment, lifetime, and production needs.
\item[Bring-up] Power, initialize, emit, lock, telemetry, and pass data on a
      known-good host. Removes integration unknowns only.
\item[Nominal characterization] Map normal performance across temperature,
      voltage, lane, wavelength, pattern, and unit: power, sensitivity, BER,
      eye or signal-quality metrics, FEC, lane variation. Distributions, not a
      hero sample.
\item[Margin characterization] How far from each failure boundary? Optical
      loss, sensitivity, electrical, timing, control authority, environmental
      headroom.
\item[Interoperability] Target hosts, ASICs, firmware, fibers, connectors,
      peer devices, and supplier combinations.
\item[Environmental and reliability qualification] Temperature, voltage,
      ripple, airflow, humidity, contamination, vibration, connector cycling,
      aging, and accelerated stress with a named mechanism and justified
      $E_a$ where life is claimed.
\item[Manufacturing and ATP readiness] Lot consistency, distributions,
      measurable limits, ATP coverage, guardbands, supplier controls,
      escape detection.
\item[Controlled pilot] Limited population, enhanced monitoring, exit
      criteria, rollback or containment plan.
\item[Fleet deployment and monitoring] BER/FEC, retrains, power, temperature,
      lane behavior, cohort trends, field failures, supplier and lot
      correlations.
\end{description}

The worked answer in \Cref{sec:interview-worked-validation} is practice prose

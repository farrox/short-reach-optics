...
Measure the network at workload, fabric, link, and module layers. At workload
level, record step time, collective latency, accelerator idle time, and tail
behavior. At fabric level, record delivered bandwidth, queue occupancy, route
balance, retries, and failed links. At link level, record pre-FEC BER, FEC error
distributions, equalizer state, and flaps. At module level, record optical power,
temperature, wavelength or lock state, and electrical power. A peak-rate test does
not replace an all-reduce or all-to-all run on the intended topology
(\Cref{sec:collectives,sec:fleet-triage}).
...
\begin{enumerate}
\item What is the difference between scale-up and scale-out, and why do they use
      different optics?
\item A single lane has rising FEC errors but its Rx power is stable. Is this a
      power problem or a signal-quality problem?
\item How do pluggable, linear pluggable, and co-packaged choices trade host margin,
      power, cooling, fiber count, and repair time?
\end{enumerate}

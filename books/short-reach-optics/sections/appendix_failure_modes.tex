...
Chapter~10 detects and bounds the operational anomaly: FEC activity, retrains,
cohorts, severity, and initial containment
(\Cref{ch:networking}). This chapter begins when the issue becomes an
investigation that must confirm mechanism and ownership. Earlier chapters own
the underlying physics (\Cref{ch:models,ch:lasers,ch:wdm}). Symptom recipes and
the fleet-bucket map live in \Cref{app:failure-analysis-reference}.
...
Chapter~10; mechanism confirmation belongs here''
...
Chapter~10 for operational detection and the FA reference for symptom recipes''

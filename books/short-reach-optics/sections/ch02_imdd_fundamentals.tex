...
of single-mode fiber, loss and dispersion are modest. A PAM4 IM/DD link with
equalization, FEC, and a reasonable laser closes without coherent mixing. The
...
optical path is passive fiber. Retimed 800G pluggables run roughly 14--18~W; LPO
trims to roughly 7--9~W by deleting module DSP~\cite{lpo}. A 400ZR-class coherent
pluggable is built around a coherent DSP that alone draws roughly 5--8~W inside a
15~W module budget~\cite{oif400zr}, landing near 45~pJ/bit at 400~Gb/s. Over a
100~m rack link that extra DSP power does not buy enough margin to justify the BOM
and service complexity (\Cref{sec:power}).
...
the signal; the receiver must undo that inter-symbol interference (ISI) before
...
\item[FFE (feed-forward equalizer)] \firstterm{FFE} is a finite-impulse-response
      ...
      Transmitter and dispersion eye closure quaternary (TDECQ) applies a
...
bottom two OSI layers: the \term{MAC} (data link) builds frames and carries the
...
Inside the PCS, the MAC's 64-bit words are first wrapped in 64B/66B blocks: a 2-bit
...
\begin{description}
\item[OMA (optical modulation amplitude)] $P_1 - P_0$: the real signal swing.
      For PAM4 the ``outer OMA'' spans the outermost levels.
\item[Extinction ratio (ER)] $P_1/P_0$, in dB. Trades against insertion loss and
      against \term{TDECQ} (\Cref{ch:validation}).
\item[RIN (relative intensity noise)] the laser's own amplitude noise; sets a
...
silicon photonics and many datacenter SMF links use 1310~nm class wavelengths. At
...
\item \textbf{Reflections:} light reflected back into the laser raises effective
      chirp and RIN; optical return loss (ORL) specs exist for this reason
...
\begin{description}
\item[SR (short reach)] multimode fiber, VCSEL at 850~nm or evolving MMF solutions;
      rack and row distances; modal noise and bandwidth limit legacy SR.
\item[DR (datacenter reach)] single-mode, typically 500~m class at 1310~nm; PAM4,
      KP4, TDECQ limits; the mainstream 400G/800G/1.6T module reach.
\item[FR (far reach)] single-mode, 2~km class; same optics family as DR with tighter
...
\item[Electrical COM] After the CEI reference TX, channel, and Rx (CTLE + 8-tap
      DFE), COM $\ge3$~dB is the usual pass line at the slicer
...
useful (eight lanes make a 3.2~Tb/s OSFP-class port), but what \emph{symbol rate}
...
A microring modulator (MRM) wraps a phase-shifter waveguide into a closed loop
...
spectral range (FSR) in a smaller footprint. Both are resonant filters as well as
...
Silicon photonics builds the Mach--Zehnder modulator (MZM) as a push-pull
interferometer on a silicon-on-insulator (SOI) rib or strip waveguide. Phase
...
200G/lane DR/FR silicon-photonics transceivers. Pushing to 400G/lane IM/DD was open
...
\term{TFLN} refers to a sub-micrometer slice of lithium niobate bonded to a
...
\term{electro-optic} (EO) bandwidth. The modulator is a Mach--Zehnder
interferometer (MZM): a \term{Pockels} phase shifter in each arm, driven in
push-pull, converts RF voltage on a \term{traveling-wave electrode} (TWE) into
...
SiGe/BiCMOS \emph{modulator driver} die, or (for LPO) the host SerDes itself.

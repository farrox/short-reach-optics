...
\item What does KP4 FEC buy, and what does it cost in overhead and latency?
\item Why does IM/DD dominate short reach while coherent wins at longer distances?
\end{itemize}

\paragraph{Design.}
\begin{itemize}
\item Which plane owns each limit, and can production reach that plane without a
      lab-only fixture?
\item Which margin dominates: OMA, TDECQ, ORL, receiver sensitivity, or host
      channel operating margin?
\item What prevents saturation at maximum launch power and minimum channel loss?
\item Which bias or equalizer settings are calibrated, and how are bad tables
      detected before ship?
\end{itemize}

\paragraph{Debug.}
\begin{itemize}
\item Average power is in spec but BER fails. What do you measure next?
\item The equalizer taps are saturated. What does that tell you about the channel?
\item FEC errors are clustered rather than random. What failure mechanisms produce
      bursts?
...
\begin{itemize}
\item What is the fastest production test that catches a bad bias calibration?
\item How do you correlate ATE TDECQ to a reference DCA?
\item What changes in the ATP when you move from retimed to LPO modules?
\end{itemize}

...
clean facet DPA does not prove the connector was innocent if ORL was never
...
\item[SMSR / EAM / RIN] Spectrum, modulator, or noise floor: which ledger moved?
...
Prevent recurrence (ATP / SPC / process)
  |
Verify next lot + fleet watch
\end{dectree}

Containment, root-cause investigation, and recurrence control are three
different actions. Apply the supplier-escape and recurrence trees in
\Cref{sec:tree-escape,sec:tree-recurrence} to separate immediate hold from
FA and from the ATP or SPC change that closes the loop. Organize the spent
margin with the five ledgers before naming a component
(\Cref{sec:laser-margin-erosion,sec:debug-fork}).
...
criteria a supplier can fail clearly, not slogans. EVT/DVT/PVT/MP are stage
...
\footnotesize
\renewcommand{\arraystretch}{1.25}%
\begin{tabular}{@{}p{0.10\linewidth}p{0.24\linewidth}p{0.34\linewidth}p{0.26\linewidth}@{}}
\toprule
...
Detailed exit examples and owners follow below.\par}
\end{table*}

\paragraph{EVT (engineering validation test).}
\textbf{Purpose:} prove the design can operate, not that it is ready for
...
\textbf{Purpose:} prove the requirements slice across temperature, voltage,
...
\textbf{Purpose:} prove the supplier can build the qualified result at yield
...
EVT is bring-up. DVT is characterization plus margin/interop plus frozen
stress intent. PVT is production readiness. MP is fleet monitoring with
manufacturing ownership. Do not use an EVT hero sample as PVT evidence.
...
Each ATP line is a screen with a purpose, not a museum of instruments. Limits
come from the requirements slice and link budget; the notes below state what
uncertainty each line removes.

\paragraph{LIV ($I_\mathrm{th.}$, slope, kink)}

\textbf{Purpose.}
Is there a kink-free bias window at rated temperature?

\textbf{Uncertainty removed.}
Without LIV you cannot separate a dead bias window from later eye or BER
fails. Ship LIV is also the baseline for wear-out triage
(\Cref{sec:laser-params,sec:wearout-modes}).

\textbf{Activities.}
Sweep current with an SMU and power meter at rated $T$. Record threshold,
slope, and kink presence.

\textbf{Exit criteria.}
\textbf{Exit when} the unit shows a kink-free window inside the bias policy.

\textbf{Decision unlocked.}
Ship, reject, or tighten bias/derate.

\paragraph{Risk if omitted.}
Kinked or weak diodes escape and age into soft BER without a compare baseline.

\paragraph{SMSR.}

\textbf{Purpose.}
Does single-mode purity meet the floor so modal noise stays controlled?

\textbf{Activities.}
Measure SMSR on an OSA versus the spec floor at temperature.

\textbf{Exit criteria.}
\textbf{Exit when} SMSR clears the floor with the program guardband.

\textbf{Decision unlocked.}
Ship or reject for modal risk; feed aging SMSR into life screens.
...
\textbf{Purpose.}
Does the noise floor survive quiet and stressed optical return loss?

\textbf{Activities.}
Measure RIN with a PD and ESA at stated ORL and bandwidth
(\Cref{sec:rin-values}).

\textbf{Exit criteria.}
\textbf{Exit when} quiet and stressed RIN meet the BER-floor budget.

\textbf{Decision unlocked.}
Ship, require isolation, or reject for reflection sensitivity.

\paragraph{Risk if omitted.}
Clean-bench parts floor under field ORL.

\paragraph{Wavelength / grid.}

\textbf{Purpose.}
Is the channel ID correct, and is $d\lambda/dT$ inside the lock budget
(\Cref{ch:wdm})?

\textbf{Activities.}
OSA or wavemeter at temperature corners.

\textbf{Exit criteria.}
\textbf{Exit when} wavelength and drift support the assigned grid and filters.

\textbf{Decision unlocked.}
Ship on channel, retune, or reject for grid miss.

\paragraph{Risk if omitted.}
WDM unlock and crosstalk appear as host or DSP bugs.

\paragraph{Optical power class.}

\textbf{Purpose.}
Does launch power meet the ELSFP / MSA class the link budget assumed?

\textbf{Activities.}
Power meter at the named reference plane.

\textbf{Exit criteria.}
\textbf{Exit when} power class is met with guardband.

\textbf{Decision unlocked.}
Ship, bin, or reject for budget failure.
...
\textbf{Purpose.}
Do ER, RLM, and TDECQ at baud close with a stable EAM bias
(\Cref{sec:tdecq})?

\textbf{Activities.}
EAM bias sweep with a DCA under named driver conditions.

\textbf{Exit criteria.}
\textbf{Exit when} eye metrics meet limits inside the bias window.

\textbf{Decision unlocked.}
Ship EML, retune bias tables, or reject modulator quality.

\paragraph{Risk if omitted.}
Average power passes while Tx quality fails under temperature and age.

\paragraph{CMIS / TWI bring-up.}

\textbf{Purpose.}
Do registers, alarms, and the state machine work so field telemetry is real?

\textbf{Activities.}
Host or CMIS tool bring-up through ready state; check alarms and key pages.

\textbf{Exit criteria.}
\textbf{Exit when} management presence, state progression, and alarm paths
pass.

\textbf{Decision unlocked.}
Ship with trusted telemetry, or hold for firmware / TWI faults.
...
\textbf{Purpose.}
Do mate cycles and endface grade meet IEC~61300 limits for the service model
(\Cref{sec:connector-reliability,sec:elsfp})?

\textbf{Activities.}
ORL meter and mate-cycle sample against rating; endface grade as required.

\textbf{Exit criteria.}
\textbf{Exit when} cycles and grade meet the claimed service life.

\textbf{Decision unlocked.}
Ship, derate mate life, or reject packaging.

\paragraph{Risk if omitted.}
Connector wear raises ORL and RIN after routine service.

\paragraph{Burn-in screen.}

\textbf{Purpose.}
Are infant-mortality parts culled before ship (\Cref{sec:gr468})?

\textbf{Activities.}
HTOL sample or 100\% screen per the reliability plan; track fallout.

\textbf{Exit criteria.}
\textbf{Exit when} the screen completes and fallout is inside control limits.

\textbf{Decision unlocked.}
Ship the lot, hold for process excursion, or tighten burn-in.

\paragraph{Risk if omitted.}
Early-life clusters hit the fleet by date code.

\paragraph{Driver / TIA ESD and latch-up.}

\textbf{Purpose.}
Is IC ESD/latch-up rating on file, and does sample audit support it
(\Cref{sec:ic-reliability})?

\textbf{Activities.}
Review supplier JESD47 / HBM / CDM reports; sample audit as required.

\textbf{Exit criteria.}
\textbf{Exit when} ratings are on file and audit finds no latch-up under the
agreed injection.

\textbf{Decision unlocked.}
Accept electronics reliability evidence, or hold for IC margin / handling.

\paragraph{Risk if omitted.}
Dark modules from handling ESD are mis-labeled as laser COD.

\paragraph{Thermal class.}

\textbf{Purpose.}
Do LIV, RIN, and CMIS still pass at the claimed case temperature?

\textbf{Activities.}
Chamber at case $T$ with the ATP subset that protects derate policy.

\textbf{Exit criteria.}
\textbf{Exit when} thermal-class ATP rows pass at the rated case $T$.

\textbf{Decision unlocked.}
Approve thermal class, derate, or reject.

...
extended with optical, reliability, and manufacturing terms used in the book
(including the must-know list in \Cref{app:interview-review}).
...
\item[8D / CAPA] Eight-discipline problem solving and corrective and preventive action. Structured containment, root cause, correction, and recurrence control for supplier or production failures.
...
\item[ATP] Acceptance test plan. Production test limits, methods, reference planes, and reaction rules for ship or hold decisions.
...
\item[Burn-in] Production or sample screen that removes infant-mortality parts before ship. Distinct from HTOL life projection.
...
\item[DPPM] Defective parts per million. Incoming or outgoing quality rate used in supplier quality and ATP escape tracking.
...
\item[DVT] Design Validation Test. Proves the design meets requirements across corners, margin, and a frozen life plan before volume tooling (\Cref{tab:npi}).
...
\item[EVT] Engineering Validation Test. Early bring-up on engineering samples: first light, CMIS, basic LIV/SMSR/RIN, one closing BER (\Cref{tab:npi}).
...
\item[MP] Mass production. Sustained volume after PVT: DPPM, RMA ownership, ECO control (\Cref{tab:npi}).
...
\item[NPI] New product introduction. Gate sequence EVT $\to$ DVT $\to$ PVT $\to$ MP that maps onto the validation ladder (\Cref{tab:npi}).
...
\item[PVT] Production Validation Test. Proves the factory can build the qualified design repeatedly at yield with ATP and SPC (\Cref{tab:npi}).
...
\item[SPC] Statistical process control. Tracks process distributions and trends to catch drift before escapes ship.

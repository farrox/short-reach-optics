...
This appendix is the interview quick-reference. Deep technical explanations live
in the body chapters and \Cref{app:interview-review}. Each playbook uses one
format: question, assumptions, 30-second answer, key concepts, common mistakes,
and deep-dive reference. Memorize the callouts labeled ``30-second answer.''
Expand only where the interview asks. Name access level
(\Cref{sec:interview-access-levels}) and use \Cref{sec:tree-evidence-block}
...
\fwwhy{Stable average power deprioritizes gross insertion loss but does not
eliminate fast fluctuation, intermittent contact, monitor averaging, or
reflection-dependent effects. Scope sets severity first; correlation after
isolation unlocks contain or lot action. A bias sweep requires engineering
access and is not universally first
(\Cref{sec:tree-debugging,sec:tree-blackbox-access}).}
...
\item ``The receiver is bad because optical power is fine.'' Power does not
      represent noise, timing, distortion, or control behavior.
\item Naming the laser first.
\item Calling ``bad eye'' a confirmed mechanism.
\item Listing instruments without a decision or recurrence control.
\end{itemize}}

\fwclose{Average power is stable, so I deprioritize gross loss, scope the
population, chase signal quality on the black-box surface first, isolate Tx /
channel / Rx / DSP, then close with the control that catches the next escape.}

\fwdeep{Full prose: \Cref{sec:interview-worked-sensitivity,sec:fm-ber-increase}.
Ledgers: \Cref{sec:interview-margin-ledgers}. Pattern:
...
\fwassumptions{The failure reproduces; telemetry is calibrated unless challenged; the product is bookended; engineering access is unavailable unless requested.}

\fwfirst{Confirm the power drop with an external meter at a named plane before trusting module monitors.}
...
\fwfirst{Confirm whether one lane is an outlier versus a multi-lane gradient or pattern.}
...
\fwfirst{Confirm scope and whether cool-down recovers before blaming the laser.}
...
\fwfirst{Confirm whether recalibration recovers the symptom (black-box) before opening LIV.}
...
\framework{Second component source}
\label{fw:second-source}
\label{fw:second-component}

\fwquestion{How would you qualify a second laser, photodiode, driver, TIA, or PIC supplier?}

\fwtesting{Component-boundary judgment: requirements at the component interface, wafer/lot distributions, integration compatibility, process-specific reliability, supplier ATP correlation, and module-level confirmation.}

\fwassumptions{The component boundary and ownership are defined; module confirmation is still required before open volume; engineering access to source samples may be available.}

\fwfirst{Freeze the component-boundary requirements slice, not the incumbent datasheet.}

\execanswer{Freeze requirements at the component boundary, not the incumbent datasheet. Characterize multi-lot distributions for LIV, SMSR, RIN, wavelength, and process corners. Demonstrate integration compatibility in the host module, run mechanism-appropriate reliability, correlate supplier ATP to your proxy, then confirm on module samples. Therefore I would gate open volume on FAIR, ATP correlation, and split RMA codes, not on a hero die.}
...
\fwwhy{Component and complete-product second sources have different access, interop burden, and owners
(\Cref{sec:tree-supplier,sec:supplier-exec,sec:tree-evidence-block}).}

\fwmeas{Wafer/lot distributions $\rightarrow$ process spread? $\rightarrow$ risk.\\
...
\fwfollow{\begin{itemize}
\item What is measured at the component boundary versus the module connector?
\item What changes if the second source is only a laser die?
\end{itemize}}

\fwmistakes{\begin{itemize}
\item Treating a hero sample as a distribution.
\item Skipping module confirmation after die-level pass.
\end{itemize}}

\fwclose{I qualify at the component boundary with multi-lot distributions, then confirm in-module with FAIR, ATP correlation, and split RMA.}

\fwdeep{\Cref{sec:interview-worked-second-source,sec:supplier-exec}.}

\framework{Second module or cable source}
\label{fw:second-module}

\fwquestion{How would you qualify a second transceiver, AOC, or cable-assembly supplier?}

\fwtesting{Complete-product second source: black-box nominal behavior, margin, host/peer interop, environmental and reliability evidence, manufacturing readiness, controlled pilot, and fleet RMA separation.}

\fwassumptions{The product is bookended unless engineering samples are contracted; customer-visible metrics dominate; open volume waits on pilot exit.}

\fwfirst{Define what equivalence means for the customer-visible product before measuring a golden sample.}

\execanswer{Freeze the customer-visible requirements slice. Walk architecture feasibility, bring-up, characterization, margin, interoperability, reliability qualification, manufacturing validation, controlled pilot, mass production readiness, then fleet monitoring with feedback. Request engineering access only when black-box evidence is insufficient. Therefore I would gate open volume on FAIR, ATP correlation, pilot exit, and split RMA codes.}
...
\fwwhy{Module second-source burden is interop and black-box margin across the envelope, not die physics first
(\Cref{sec:tree-blackbox-access,sec:interview-validation-ladder}).}
...
\fwfollow{\begin{itemize}
\item Why not clone the incumbent datasheet?
\item What changes between a pluggable and an AOC second source?
\end{itemize}}

\fwmistakes{\begin{itemize}
\item Qualifying a hero module.
\item Merging supplier RMA codes.
\end{itemize}}

\fwclose{I walk the product lifecycle on the customer-visible surface, then gate volume on FAIR, ATP correlation, pilot exit, and split RMA.}

\fwdeep{\Cref{sec:interview-worked-second-source,sec:supplier-exec}.}

\framework{Product-readiness plan for a new transceiver}
\label{fw:validation-plan}

\fwquestion{How would you establish readiness for a new optical transmitter from
bring-up through production?}

\fwtesting{Product-readiness judgment: sequencing, ownership, stage gates, dependencies, exit criteria, schedule and resource judgment.}

\fwassumptions{Requirements and owners exist; stage exits are binary enough to fail; engineering access is requested only when a gate cannot decide.}

\fwfirst{Freeze the requirements slice and named risks before listing instruments.}

\execanswer{I start by defining the system requirements and failure risks. Then I build the product-readiness plan around reducing uncertainty: architecture demonstrates the budgets can close, bring-up demonstrates basic operation, characterization builds the behavioral model, requirement verification and system validation close specs and intended use, reliability qualification demonstrates life and environmental confidence, manufacturing validation demonstrates repeatability, and controlled deployment plus telemetry confirms field assumptions. Every stage has an exit criterion tied to a decision.}
...
\fwwhy{This answer is about program sequencing and ownership. Qualification
\emph{evidence} construction lives in \Cref{fw:qual-plan,sec:tree-qual-evidence}.
The product-readiness stages are the gates
(\Cref{sec:interview-validation-ladder,sec:tree-qualification}).}

\fwmeas{Each readiness stage $\rightarrow$ named uncertainty removed
...
\fwclose{I walk the ladder as staged uncertainty reduction and refuse any measurement that answers no new question about remaining margin.}
...
\fwfirst{Scope population and trend (unit / lot / vendor / rack / fleet) before pulling hardware.}
...
\heuristic{Contain a growing lot-scoped escape before you finish the physics
story. The next day of ship can cost more than the next day of FA.}
...
\fwfirst{Provisionally contain the growing population, then refine scope.}
...
\fwmistakes{\begin{itemize}
\item Waiting for a confirmed mechanism before stop-ship.
...
\fwfirst{Confirm floor versus waterfall shift with a VOA sweep at a named plane.}
...
\fwfollow{\begin{itemize}
\item Why not increase OMA alone?
...
\fwfirst{Preserve the failing state and telemetry before reseat, clean, or reboot.}
...
\framework{Production recurrence-control update}
\label{fw:atp-update}

\fwquestion{A failure escaped to the field that production controls did not catch. How do you update recurrence control?}

\fwtesting{Choosing among ATP, sampling, SPC, supplier process, design, firmware, and telemetry controls; not assuming a new 100\% screen.}

\fwassumptions{The escape path can be named; a failing unit or equivalent can be replayed; measurement capability is known.}

\fwfirst{Name the escape path and the measurement that would have caught it.}

\execanswer{Name the escape path and the measurement that would have caught it at a named plane and corner. Choose the most economical control: new ATP item, tighter limit, sampled audit, SPC variable, supplier process correction, design or firmware guard, or telemetry alarm. Only select a 100\% screen when separation, cycle time, measurement capability, and cost justify it. Therefore I would ship the chosen control with an owner and a metric.}
...
\fwwhy{Recurrence control is not always an ATP change. A limit nobody can
trace gets waived under ship pressure
(\Cref{sec:interview-worked-validation,sec:tree-escape}).}
...
\fwclose{I name the missed uncertainty, choose ATP / sample / SPC / supplier / design / firmware / telemetry, demonstrate separation, and ship the control with an owner.}
...
\fwassumptions{Schema and firmware versions are known; retention and owners exist; missing-data behavior is defined; alarms have a decision owner.}

\fwfirst{Name the decision each telemetry field must unlock before adding registers.}

\execanswer{Log what discriminates hypotheses: per-lane power, bias, pre-FEC BER and FEC histograms; module temperature and actuator drive; LOS/LOL with context. Require timestamp accuracy, sampling cadence, aggregation window, units, calibration/scaling, missing-data behavior, firmware/schema version, serial/lot/platform, event trigger, retention, and a decision owner. Alarm on trends and disagreements, not only hard thresholds. Therefore I would instrument the ledgers margin testing said die first.}

Add a register only if it changes contain, derate, RMA, or FA ownership. Full
tradeoff: \Cref{sec:fleet-triage}.
...
\fwtesting{Qualification-evidence judgment: mechanism coverage, stress selection, sample strategy, acceptance criteria, confidence, production correlation, release evidence. This playbook is a scoped evidence path, not a second full lifecycle; use \Cref{fw:validation-plan,sec:tree-qualification} for the stage order.}

\fwassumptions{Requirements name mechanisms; sample strategy and confidence are stated; production proxies are cheaper than full GR-468 replay; release is a ship / restrict / reject decision.}

\fwfirst{Freeze the requirements slice and named mechanisms before listing stresses.}

\execanswer{I qualify from requirements to mechanisms, not from a ritual list. What does the system require? What can fail? How do we accelerate those mechanisms? What observable changes? What defines pass/fail? How much evidence is enough? How does production control maintain quality? Therefore I would gate ship on remaining margin after named stresses with a stated sample strategy and confidence, plus a production proxy (\Cref{sec:tree-qual-evidence,sec:qual-planning-matrix}).}
...
\fwwhy{This is a scoped evidence path inside the lifecycle, not a second
lifecycle (\Cref{fw:validation-plan,sec:tree-qualification}). Stress consumes
margin; qual measures what remains
(\Cref{sec:interview-margin-budget,sec:interview-customer-vendor}). Sample
strategy depends on failure-rate target, confidence, cost, and population
variation, not a fixed ``we test 20 units.''}
...
\fwfirst{State evidence, scope, and residual risk before waiting for a physical mechanism.}
...
\section{Tradeoff interview questions}
\label{sec:interview-tradeoff-qs}

Staff follow-ups often stop asking ``what test?'' and start asking ``given
constraints, what do you choose?'' Answer each with benefit, downside, and
decision criteria. Point at the matching tradeoff homes; do not invent a new
framework.

\paragraph{Would you choose more margin or lower power?}
\textit{Benefit of margin:} reach, temperature, aging, contamination tolerance.\\
\textit{Downside:} laser power, heat, efficiency, sometimes lifetime.\\
\textit{Criteria:} allocate margin where uncertainty is highest; do not maximize
every ledger (\Cref{sec:laser-margin-erosion,ch:validation}).

\paragraph{Would you add more telemetry?}
\textit{Benefit:} faster fleet triage and earlier margin erosion detection.\\
\textit{Downside:} firmware, storage, alarm fatigue.\\
\textit{Criteria:} each field needs a decision owner and a reaction plan
(\Cref{fw:telemetry,sec:fleet-triage}).

\paragraph{Would you run more qualification?}
\textit{Benefit:} confidence and fewer late escapes.\\
\textit{Downside:} schedule, cost, delayed learning.\\
\textit{Criteria:} prioritize by risk $\times$ uncertainty $\times$ impact; stop
when remaining risk has a production control (\Cref{sec:qual-sample-strategy}).

\paragraph{Would you add a second supplier?}
\textit{Benefit:} supply resilience and pricing leverage.\\
\textit{Downside:} validation, interop, and manufacturing differences.\\
\textit{Criteria:} qualify on concentration risk and evidence, not ideology
(\Cref{sec:supplier-exec,fw:second-module}).

\paragraph{Would you increase ATP coverage?}
\textit{Benefit:} escape detection earlier in the flow.\\
\textit{Downside:} cycle time, cost, false rejects.\\
\textit{Criteria:} cheapest control that reliably detects the named mechanism:
100\%, sample, SPC, or supplier process (\Cref{sec:hvm-test}).

\keyidea{Open the matching framework, deliver the thirty-second box, walk the
tree, end on the decision and the control. When the interviewer asks a tradeoff
question, name benefit, downside, and criteria. Philosophy is in
